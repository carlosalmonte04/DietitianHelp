...
$(document).ready(function () {
  ...
  $("#comply-btn").on("click", function moveToComply() {
    ...
    fetch(
      "/clients/" + client.id + "/fullassessments",
      fullassessmentRequestData,
    )
      .then((res) => res.json())
      .then((fullassessment) => {
        $("#modal-alert2").iziModal("open");
        $("#complyTopTab").trigger("click");
      });
  });
  ...
  top
    ...
    .text("<%=@client.firstname[0].to_s +  @client.lastname[0].to_s %> Loaded");
  ...
  if (top.$("#historyFromMenu").length == 0) {
    top
      .$(".gn-submenu")
      .append(
        "<li id='historyFromMenu' data-clientId=" +
          "'" +
          top.$currentClient.id +
          "'" +
          "onclick='getHistory()'><a type='button' aria-expanded='false'>History</a></li>",
      );
  }
  ...
  $("#add-goal").submit(function (event) {
    ...
    const revisionDate = new Date($("#goal-revision-date")[0].value)
      .toISOString()
      .slice(0, 10)
      .replace(/-/g, "/");
    ...
    if (goalName.length > 3) {
      ...
      fetch("/clients/" + client.id + "/goals", goalsRequestData)
        ...
        .then(() => {
          ...
          $('[value="Add Goal"]').removeAttr("disabled");
        });
    }
  });
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (stat == "archived") {
        ...
        $($("#item-" + goalID).find(".checkbox")).attr(
          "onclick",
          "'updateGoal('" + url + "', " + goalID + ", " + "'complete')'",
        );
        $("#item-" + goalID.toString())
          .closest(".list-group-item")
          .removeClass("animated tada loading checked-todo")
          .addClass("archive-item");
        $($("#item-" + goalID).find(".fa")[0])
          .removeClass("fa-archive")
          .addClass("fa-check")
          .attr(
            "onclick",
            "updateGoal('" + url + "', " + goalID + ", " + "'active')",
          );
      }
      if (stat == "active") {
        $(":checkbox#" + goalID.toString()).prop("checked", false);
        ...
        $("#item-" + goalID.toString())
          .closest(".list-group-item")
          .removeClass("archive-item checked-todo animated tada loading");
        $($("#item-" + goalID).find(".fa")[0])
          .removeClass("fa-check")
          .addClass("fa-archive")
          .attr(
            "onclick",
            "updateGoal('" + url + "', " + goalID + ", " + "'archived')",
          );
      }
      if (stat == "complete") {
        ...
        $("#item-" + goalID.toString())
          .closest(".list-group-item")
          .removeClass("archive-item checked-todo animated tada loading")
          .addClass("checked-todo");
        $($("#item-" + goalID).find(".fa")[0])
          .removeClass("fa-check fa-archive")
          .addClass("fake-icon")
          .attr(
            "onclick",
            "'updateGoal('" + url + "', " + goalID + ", " + "'active')'",
    ...
    $.ajax({
      ...
      data: {
        nextevaluationnote: {
          ...
          nnote_date: new Date($("#note-editor-1")[0].value)
            .toISOString()
            .slice(0, 10)
            .replace(/-/g, "/"),
          client_id: client.id,
      ...
      [
        ...
        function (instance, toast) {
          ...
          $.ajax({
            ...
            success: function (data) {
              top.document
                .getElementById("clientload")
                .contentDocument.getElementById("clientloadright").innerHTML =
                data;
              instance.hide();
            },
          });
